...
**After adding a new `rust_library` target to BUILD.bazel:**

`rust-project.json` (generated by Bazel) tells rust-analyzer about the project's crate graph. After adding a new target, regenerate it first, then kill rust-analyzer so all consumers (editor and Claude Code's LSP tool) restart and pick up the new file:
