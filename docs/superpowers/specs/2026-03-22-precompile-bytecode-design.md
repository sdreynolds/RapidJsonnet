...
Add an optional `precompile_bytecode` boolean attribute to the `jsonnet_library` Bazel rule. When set to `True`, the rule runs `//:jsonnet_aot` on the source file to produce a `.jsonnetc` bytecode artifact. Both the raw source and the precompiled bytecode are passed through in the transitive depsets, allowing the VM to pick up the cached bytecode at import time.
...
### Prerequisite: `jsonnet_aot` CLI change

The current `jsonnet_aot` binary hardcodes output next to the input file (`format!("{}c", filename)`). This is incompatible with Bazel's sandboxed actions where inputs are read-only and outputs must be pre-declared. The compiler must be modified to accept an explicit output path: `jsonnet_aot <input> <output>`. When only one argument is provided, the existing behavior (writing next to input) is preserved for backwards compatibility.
...
2. The rule runs `//:jsonnet_aot` via `ctx.actions.run` with both the input source path and the declared output path as arguments.
...
A new private `_compiler` attribute is added to `jsonnet_library`, pointing to `//:jsonnet_aot` with `executable = True` and `cfg = "exec"`. Named `_compiler` (not `_tool`) to distinguish from `jsonnet_to_json`'s `_tool` which points to `//:main`.
